...
// num of bytes of code and of data, 4 bytes for code, 8 bytes for data
...
char *globalOutputIntermediate;
char *globalOutputBinary;
...
// 0 for data mode, 1 for code, -1 for unset
int type = -1;
...
int main(int argc, char *argv[])
{
	...
	char *intermediateOutputFilePath = "./temp"; // argv[2];
	char *binaryOutputFilePath = argv[2];
	globalOutputBinary = binaryOutputFilePath;
	globalOutputIntermediate = intermediateOutputFilePath;
	...
	if (!IS_CURRENTLY_TESTING)
	{
		...
		// printf("code: %li data: %li\n", code_seg_size, data_seg_size);
		//  fprintf(fptr, "%s", )
...
{
	remove(globalOutputBinary);
	remove(globalOutputIntermediate);
...
void processIntermediateIntoBinary(char *intermediateFilePath, char *outputBinaryFilePath)
{
	...
	fwrite(&file_type, 8, 1, outputFile);
	fwrite(&code_seg_begin, 8, 1, outputFile);
	fwrite(&code_seg_size, 8, 1, outputFile);
	fwrite(&data_seg_begin, 8, 1, outputFile);
	fwrite(&data_seg_size, 8, 1, outputFile);
...
uint8_t parseReg(char *registry)
{
	...
	char *end;
	errno = 0;
	unsigned long regNum = strtoul(registry + 1, &end, 0);

	if (regNum > 31 || errno != 0 || *end != '\0' || end == registry)
	{
